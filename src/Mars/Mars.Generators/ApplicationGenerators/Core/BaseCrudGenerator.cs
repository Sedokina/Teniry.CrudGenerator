    ...
    where TConfiguration : CqrsConfiguration
{
    ...
    protected BaseCrudGenerator(
        ...
        : base(context,
            entityScheme.Configuration.AutogeneratedFileText,
            entityScheme.Configuration.NullableEnable)
    {
        DbContextScheme = dbContextScheme;
        EntityScheme = entityScheme;
        Configuration = configuration;
        Symbol = symbol;
        BusinessLogicNamespace = entityScheme.Configuration.BusinessLogicNamespaceBasePath.GetNamespacePath(
            EntityScheme.EntityName,
            Symbol.ContainingAssembly.Name,
            entityScheme.Configuration.FeatureNameConfiguration,
            configuration.FunctionName);
        EndpointNamespace = entityScheme.Configuration.EndpointsNamespaceBasePath.GetNamespacePath(
            EntityScheme.EntityName,
            Symbol.ContainingAssembly.Name);
    }
    ...
    protected override void WriteFile(string templatePath, object model, string className)
    {
        ...
        var usingNamespaces = new[] { DbContextScheme.DbContextNamespace, EntityScheme.EntityNamespace }
            .Distinct()
            .Select(x => $"using {x};");
        ...
        var sb = new StringBuilder();
        sb.AppendLine(AutogeneratedFileText);
        if (EntityScheme.Configuration.NullableEnable)
        {
            sb.AppendLine("#nullable enable");
        }
